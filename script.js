...
async function loadHistoryRecord(id, options = {}) {
    ...
    try {
        ...
        // ไปแสดงฟอร์มพร้อมข้อมูลที่โหลด (ยกเว้นตอนเรียกใช้เพื่อ "ปริ้น" อย่างเดียว
        // ซึ่งไม่ต้องการสลับหน้าจอไปฟอร์มเลย)
        if (!skipNavigate) {
            setTimeout(() => {
                goToForm();
            }, 200);
    ...
    async function printHistoryRecord(id) {
        await loadHistoryRecord(id, { skipNavigate: true });
        generatePrintView();
    }
